...
IDLE_RESET_TIME = 4.0      # Seconds of no input before resetting
...
class PlayerState:
    ...
    def __init__(self, color, window_h, wave_w, wave_speed):
        ...
        # Wave position state (original logic — untouched)
    ...
    def update(self, dt, current_time, amplitude):
        ...
        # --- idle reset ---
        if current_time - self.last_input_time > IDLE_RESET_TIME:
            self.last_signal      = None
            self.prev_accept_time = None
            self.frequency        = 0.0
            self.ever_had_input   = False
            self.cycle_active     = False
            self.cos_angle        = math.pi / 2
            self.cos_speed        = 0.0
            self.arrow_speed      = 0.0

        # --- advance wave angle (bouncing 0↔π) ---
        if self.cycle_active and self.cos_speed > 0:
        ...
        # --- advance arrow angle (monotonically increasing, CCW) ---
        ...
        new_y = (self.window_h / 2) + amplitude * y_norm
...
class MuseumSyncGame(pyglet.window.Window):
    def __init__(self):
        ...
        # Phasor background (drawn before web so web appears on top of bg)
    ...
    def update(self, dt):
        ...
        p1_idle = not self.p1.ever_had_input or (now - self.p1.last_input_time > 0.3)
        p2_idle = not self.p2.ever_had_input or (now - self.p2.last_input_time > 0.3)
